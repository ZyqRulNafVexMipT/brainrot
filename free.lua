...
local function getTargetWithAI()
    ...
    local function calculatePrediction(targetHead, velocity)
        ...
        local timeToImpact = distance / 1500
...
-- (all original default config assignments preserved exactly)
...
-- ORIGINAL AIMBOT IMPLEMENTATION (kept for legacy) --------------------------
local aimbotLoop = false
local aimbotConnection = nil

-- ORIGINAL FOV CIRCLE (kept for legacy) -------------------------------------
...
local function startAutoPickUpAmmo()
    ...
    task.spawn(function()
        local pickUpAmmo = ReplicatedStorage.Network.Remotes.PickUpAmmo
        local ammoFolder = workspace.IgnoreThese.Pickups.Ammo
...
local function startAutoPickUpCoins()
    ...
    task.spawn(function()
        ...
        local lootFolder = workspace.IgnoreThese.Pickups.Coins
...
local function startAutoOpenChest()
    ...
    task.spawn(function()
        while autoOpenChestLoop do
            ...
            ReplicatedStorage.Network.Remotes.OpenCase:InvokeServer(unpack(args))
            task.wait(5)
        end
    end)
end
...
local function startAutoSpin()
    ...
    task.spawn(function()
        while autoSpinLoop do
            ReplicatedStorage.Network.Remotes.SpinWheel:InvokeServer({})
            task.wait(5)
        end
    end)
end
...
local function startAutoPickUpWeapons()
    ...
    task.spawn(function()
        local weaponsFolder = workspace.IgnoreThese.Pickups.Weapons
...
-- NEW TOGGLES ADDED TO ORIGINAL LAYOUT --------------------------------------
...
AddRightGroupbox:AddToggle("AutoSpawn", {
    Text = "Auto Spawn",
    Default = false,
    Tooltip = "Automatically respawn when you die",
    Callback = function(Value)
        if Value then startAutoSpawn() else stopAutoSpawn() end
    end
})

AddRightGroupbox:AddToggle("AutoPlaytime", {
    Text = "Auto Collect Playtime Award",
    Default = false,
    Tooltip = "Automatically collects all playtime rewards",
    Callback = function(Value)
        if Value then startAutoPlaytime() else stopAutoPlaytime() end
    end
})
...
AddRightGroupbox:AddToggle("AutoPickUpHeal", {
    ...
    Callback = function(Value)
        if Value then startAutoPickUpHeal() else stopAutoPickUpHeal() end
    end
})
...
AddRightGroupbox:AddToggle("AutoPickUpAmmo", {
    ...
    Callback = function(Value)
        if Value then startAutoPickUpAmmo() else stopAutoPickUpAmmo() end
    end
})
...
AddRightGroupbox:AddToggle("AutoPickUpCoins", {
    ...
    Callback = function(Value)
        if Value then startAutoPickUpCoins() else stopAutoPickUpCoins() end
    end
})
...
AddRightGroupbox:AddToggle("AutoOpenChest", {
    ...
    Callback = function(Value)
        if Value then startAutoOpenChest() else stopAutoOpenChest() end
    end
})
...
AddRightGroupbox:AddToggle("AutoSpin", {
    ...
    Callback = function(Value)
        if Value then startAutoSpin() else stopAutoSpin() end
    end
})
...
AddRightGroupbox:AddToggle("AutoPickUpWeapons", {
    ...
    Callback = function(Value)
        if Value then startAutoPickUpWeapons() else stopAutoPickUpWeapons() end
    end
})
...
-- ORIGINAL UNLOAD BUTTON (unchanged) ---------------------------------------
...
-- ORIGINAL AUTO-START HOOK (unchanged) -------------------------------------
